...
> = string extends Str
	...
	: Str extends `${infer first}${infer second}${infer rest}`
		? `${first}${second}` extends infer key extends
			...
			? TRomanToInt<rest, Sum<res, (typeof subtractiveNumerals)[key]>>
			: first extends infer key extends keyof typeof numerals
				? TRomanToInt<`${second}${rest}`, Sum<res, (typeof numerals)[key]>>
				: never
		: Str extends `${infer first}${infer rest}`
			? first extends infer key extends keyof typeof numerals
				? TRomanToInt<rest, Sum<res, (typeof numerals)[key]>>
				: never
			: Str extends ""
				? res
				: never;

export const romanToInt = <Str extends string>(val: Str): TRomanToInt<Str> => {
	let res: number = 0;
