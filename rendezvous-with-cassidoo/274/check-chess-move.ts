...
export const isValidMove = ({
	...
	try {
		...
		if (piece === '~') {
			throw new Error(CHESS_ERRORS.emptySpace);
		}
		...
		if (col === toCol && row === toRow) {
			throw new Error(CHESS_ERRORS.noMove);
		}
		...
		if (piece === 'p' || piece === 'P') {
			...
			if (!isMove && !isCurrentKill) {
				throw new Error(CHESS_ERRORS.badPawn);
			}
			...
			if (col !== toCol && row !== toRow) {
				throw new Error(CHESS_ERRORS.badRook);
			}
			...
			if (!isValid) {
				throw new Error(CHESS_ERRORS.badKnight);
			}
			...
			if (!isValid) {
				throw new Error(CHESS_ERRORS.badKing);
			}
